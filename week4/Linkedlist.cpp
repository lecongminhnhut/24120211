...
// 9. Remove an integer after a value of a given List:
    void removeAfter(List* &L, int val)
    {
        ...
        {
            if (cur->key == val && cur -> p_next ->p_next != NULL)
            {
            ...
            if (cur->key == val && cur->p_next == L->p_tail)
            {
                removeTail(L);
                return;
            }
...
void removePos(List* &L, int pos)
{
    ...
    if (pos == 0)
    {
        NODE* del = L->p_head;
        L->p_head = L->p_head->p_next;
        if (L->p_head == NULL) // nếu sau khi xóa mà list rỗng
            L->p_tail = NULL;
        delete del;
        return;
    }
    ...
    NODE* del = prev->p_next;
    prev->p_next = del->p_next;

    if (del == L->p_tail) // nếu đang xóa phần tử cuối
        L->p_tail = prev;
...
// 17. Remove all duplicates from a given List:
    void removeDuplicate(List* &L)
    {
        if (L == NULL || L->p_head == NULL)  // Kiểm tra danh sách rỗng
            return;

        NODE* cur = L->p_head;  // Con trỏ duyệt qua danh sách
        while (cur != NULL)  // Duyệt qua tất cả các phần tử
        {
            NODE* check = cur;  // Con trỏ phụ để kiểm tra các node phía sau 'cur'
        
            // Duyệt qua các node phía sau 'cur'
            while (check->p_next != NULL)
            {
                if (check->p_next->key == cur->key)  // Nếu tìm thấy phần tử trùng lặp
                {
                    // Xóa phần tử trùng lặp
                    NODE* temp = check->p_next;
                    check->p_next = check->p_next->p_next;  // Bỏ qua node trùng
                    delete temp;  // Giải phóng bộ nhớ
                }
                else
                {
                    check = check->p_next;  // Di chuyển con trỏ 'check' đến phần tử tiếp theo
                }
            }
            cur = cur->p_next;  // Di chuyển con trỏ 'cur' đến phần tử tiếp theo
        }
    }
...
//18. Remove all key value from a given List:
    bool removeElement(List* &L, int key)
    {
        if (L == NULL || L->p_head == NULL)  // Danh sách rỗng
        return false;
    ...
    if (cur->key == key)
    {
        ...
        // Nếu chỉ có 1 node, cập nhật luôn tail
        if (L->p_tail == cur)
            L->p_tail = NULL;
    ...
    // Duyệt danh sách để tìm node có key cần xóa
    ...
    if (cur == NULL)  // Không tìm thấy
        return false;

    // Xóa node tìm được
    prev->p_next = cur->p_next;

    // Nếu node bị xóa là tail thì cập nhật lại tail
    if (cur == L->p_tail)
        L->p_tail = prev;
